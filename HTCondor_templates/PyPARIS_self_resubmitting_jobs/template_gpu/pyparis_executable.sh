...
EOS_PATH="/eos/insert/eos/path"
SIM_PATH="$EOS_PATH"
CONTAINER_PATH="./ecloud-containers-v0.0.6-cuda.sif"
...
transfer_inputs() (
	...
	apptainer exec --nv --env PYECL_USE_GPU=1 --env CUDA_VISIBLE_DEVICES=0,1 --env PYTHONNOUSERSITE=1 --home "$JOB_PWD" "$CONTAINER_PATH" bash -lc 'date; echo $ECLOUD_CONTAINER_VERSION' >> envinfo.txt
...
apptainer exec  --nv --env PYTHONNOUSERSITE=1 --env PYECL_USE_GPU=1 --env CUDA_VISIBLE_DEVICES=0,1 \
				--home "$JOB_PWD" \
				"$CONTAINER_PATH" \
